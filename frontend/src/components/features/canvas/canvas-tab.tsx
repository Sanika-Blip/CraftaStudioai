...
function CanvasTabInner({
   ...
   * Generates different structures based on prompt keywords to demonstrate dynamic rendering.
   */
  const handlePromptSubmit = useCallback(async (msg: string) => {
    ...
    if (isDemoMode) {
      ...
      if (prompt.includes("auth") || prompt.includes("login")) {
        newBlocks = [
          { id: "blk-clerk", type: "block", title: "Clerk Integration", stack: "Next.js", status: "running", subBlocks: [{ id: "s1", type: "feat", title: "OAuth Providers" }] },
        ...
        newBlocks = [
          { id: "blk-stripe", type: "block", title: "Stripe Gateway", stack: "Node.js", status: "running", subBlocks: [{ id: "p1", type: "feat", title: "Webhooks" }] },
          { id: "blk-cart", type: "block", title: "Cart Logic", stack: "React Query", status: "done" },
          { id: "blk-products", type: "block", title: "Product Catalog", stack: "Drizzle", status: "idle" }
        ];
      } else {
        newBlocks = [
          ...MOCK_PAYLOAD.blocks.slice(0, 3),
          { id: "blk-new-" + Date.now(), type: "block", title: "Generated Module", stack: "AI Optimized", status: "running" }
        ];
      }
      ...
      try {
        ...
        await fetch(`${apiUrl}/api/workflow/run`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`
          },
          body: JSON.stringify({ projectId, prompt: msg })
        });
        // Real block rendering will eventually rely on WS or polling /api/projects/:id/blocks
      } catch (err) {
        console.error("Failed to run workflow:", err);
      }
    }
  }, [setIsChatSidebarOpen, isDemoMode, projectId, getToken]);
